...
(defn- should-build?
  ...
  (let [{:keys [temp-inputs temp-outputs real-inputs real-outputs opts]} (branch-adjust-step step false)
...
(defn- assoc-log-dir
  "Associates log path to the step"
  [parse-tree steps]
  (map (fn [step]
         (assoc step
                :log-dir
                (:dir (get (:steps parse-tree) (:index step)))))
       steps))

(defn- assoc-step-cmds
  "Associates cmds to the step"
  [parse-tree steps]
  (map (fn [step]
         (let [cmds (:cmds (get (:steps parse-tree) (:index step)))]
           (assoc step :cmds (into [] (map #(str/join %) cmds)))))
       steps))
    ...
    (if (empty? steps)
      ...
      (do
        ...
        (let [steps-data (->> steps
                           ...
                           (assoc-log-dir parse-tree)
                           (assoc-step-cmds parse-tree)
